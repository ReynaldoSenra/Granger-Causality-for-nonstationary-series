...
toda_yamamoto<-function(x, n = 5, w = 1, lag.max = 10, type = c("const", "trend", "both", "none"), m = 1, o = 7, corrhet = 3, season = NULL, exogen = NULL, digits = 4, probability = 0.05) { 
  tablavar<-matrix(nrow = (n-1)*(ncol(x)-n-w+1), ncol = (12+2*w)) 
  ...
  for (i in 2:n){
    ...
    breush<-serial.test(elvar, lags.bg = corrhet, type = "ES")
    hetero<-arch.test(elvar, lags.single = 0, lags.multi = corrhet, multivariate.only = TRUE)
    if ((breush$serial[[3]] <= probability) | (hetero$arch.mul$p.value[[1]] <= probability)) {
      repeat {
        ellag<-ellag+1
        elvar<-VAR(datavar, p = ellag, type, exogen)
        breush<-serial.test(elvar, lags.bg = corrhet, type = "ES")
        hetero<-arch.test(elvar, lags.single = 0, lags.multi = corrhet, multivariate.only = TRUE)
        if ((ellag == (firstlag+o)) | ((breush$serial[[3]] > probability)  & (hetero$arch.mul$p.value[[1]] > probability))){ 
          if ((breush$serial[[3]] <= probability) | (hetero$arch.mul$p.value[[1]] <= probability)) {
    ...
    tablavar[(i-2)*(ncol(x)-n-w+1)+1,1:10]<-c(ncol(datavar), optlag[[1]][1], optlag[[1]][2], optlag[[1]][3], optlag[[1]][4], firstlag, ellag, round(breush$serial[[3]], digits), round(bera$jb.mul$Kurtosis$p.value, digits), round(hetero$arch.mul$p.value[[1]], digits))
    for (a in 1:w){
      wald1<-wald.test(b=coef(var_TD$varresult[[1]]), Sigma=vcov(var_TD$varresult[[1]]), Terms=seq((a+1),((w+1)*ellag),(w+1)))
      wald2<-wald.test(b=coef(var_TD$varresult[[(a+1)]]), Sigma=vcov(var_TD$varresult[[(a+1)]]), Terms=seq(1,((w+1)*ellag),(w+1)))
      tablavar[(i-2)*(ncol(x)-n-w+1)+1,10+a]<-round(wald1$result[[1]][3], digits)
      tablavar[(i-2)*(ncol(x)-n-w+1)+1,10+w+a+1]<-round(wald2$result[[1]][3], digits)
    }          
    ...
    for (j in (n+w+1):ncol(x)){
      ...
      var_TD<-VAR(datavar, p = (ellag + m), type, exogen)
      tablavar[(i-2)*(ncol(x)-n-w+1)+j-n-w+1,1:10]<-c(ncol(datavar), optlag[[1]][1], optlag[[1]][2], optlag[[1]][3], optlag[[1]][4], firstlag, ellag, round(breush$serial[[3]], digits), round(bera$jb.mul$Kurtosis$p.value, digits), round(hetero$arch.mul$p.value[[1]], digits))
      for (v in 1:(w+1)){
        wald1<-wald.test(b=coef(var_TD$varresult[[1]]), Sigma=vcov(var_TD$varresult[[1]]), Terms=seq((v+1),((w+2)*ellag),(w+2)))
        wald2<-wald.test(b=coef(var_TD$varresult[[(v+1)]]), Sigma=vcov(var_TD$varresult[[(v+1)]]), Terms=seq(1,((w+2)*ellag),(w+2)))
        waldi1[[v]]<-round(wald1$result[[1]][3], digits)
        waldi2[[v]]<-round(wald2$result[[1]][3], digits)
        tablavar[(i-2)*(ncol(x)-n-w+1)+j-n-w+1,10+v]<-round(wald1$result[[1]][3], digits)
        tablavar[(i-2)*(ncol(x)-n-w+1)+j-n-w+1,10+w+v+1]<-round(wald2$result[[1]][3], digits)
      }
